...
public class Niveau3 {
    ...
    public void rendu(Graphics g){
        ...
        for (int i = 4 * GROSSEUR_UNITE; i < HAUTEUR_ECRAN; i++){
            if(i < GamePanel.blocY && GamePanel.blocX == 4 * GROSSEUR_UNITE) {
                g.drawRect(4 * GROSSEUR_UNITE, i, 2, 2);
                g.setColor(Color.RED);
                if(GamePanel.blocY > 4 * GROSSEUR_UNITE) {
                    for (int j = LARGEUR_ECRAN; j < GamePanel.blocX; j++) {
            ...
            else if (GamePanel.blocX != 4 * GROSSEUR_UNITE){
                g.drawRect(4 * GROSSEUR_UNITE, i, 2, 2);
                g.setColor(Color.RED);
            }
        }
    ...
    public void collision(){
        ...
        if (GamePanel.blocY > GROSSEUR_UNITE * 3 && GamePanel.blocX == GROSSEUR_UNITE * 4) {
            for (int i = 3 * GROSSEUR_UNITE; i < GamePanel.blocY; i++) {
            ...
            for (int j = 0; j < GamePanel.blocX; j++) {
                ...
                else if (GamePanel.playerY == GamePanel.blocY - GROSSEUR_UNITE && GamePanel.playerX == j - 1){
                    GamePanel.mursD = true;
                    GamePanel.gameOver();
                }
            }
            ...
            for (int i = HAUTEUR_ECRAN; i > GROSSEUR_UNITE * 3; i--) {
                if (GamePanel. playerX == GROSSEUR_UNITE * 3 && GamePanel.playerY == i - 1) {
                    GamePanel.mursR = true;
                    GamePanel.gameOver();
                }
    ...
    public void finNiveau(){
        if (GamePanel.playerY == porteY && GamePanel.playerX == porteX && check) {
            GamePanel.menu();
